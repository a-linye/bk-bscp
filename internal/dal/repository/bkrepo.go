...
func (c *bkrepoClient) buildProject(kt *kit.Kit) (string, error) {
	...
	if cc.G().FeatureFlags.EnableMultiTenantMode {
		...
		return fmt.Sprintf("%s.%s", kt.TenantID, c.project), nil
	}
...
func (c *bkrepoClient) Upload(kt *kit.Kit, sign string, body io.Reader) (*ObjectMetadata, error) {
	...
	opt := &repo.NodeOption{Project: project, BizID: kt.BizID, Sign: sign}
...
func (c *bkrepoClient) Download(kt *kit.Kit, sign string) (io.ReadCloser, int64, error) {
	...
	node, err := repo.GenNodePath(&repo.NodeOption{Project: project, BizID: kt.BizID, Sign: sign})
	if err != nil {
		return nil, 0, err
	}
...
func (c *bkrepoClient) CompleteMultipartUpload(kt *kit.Kit, sign string, uploadID string) (*ObjectMetadata, error) {
	...
	node, err := repo.GenBlockNodePath(&repo.NodeOption{Project: project, BizID: kt.BizID, Sign: sign})
	if err != nil {
		return nil, err
	}
