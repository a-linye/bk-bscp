	...
	}{
		{
			...
			params: BuildProcessOperateParams{
				...
				ProcessInfo: table.ProcessInfo{
					...
					User:        "appuser",
					StartCmd:    "start.sh ${inst_id}",
					StopCmd:     "stop.sh ${inst_id}",
					RestartCmd:  "restart.sh ${inst_id}",
					ReloadCmd:   "reload.sh ${inst_id}",
					FaceStopCmd: "kill.sh ${inst_id}",
					Timeout:     30,
				},
			},
			wantErr: false,
