...
// applyBizIDsFlag parses the required --biz-ids flag and writes the result into cfg.
func applyBizIDsFlag() error {
	...
	if len(ids) == 0 {
		return fmt.Errorf("--biz-ids must contain at least one valid business ID")
	}
...
- Config instances`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err = m.CheckAlreadyMigrated(); err != nil {
			fmt.Printf("Pre-migration check failed: %v\n", err)
			os.Exit(1)
		}
...
// preflightCmd represents the preflight connectivity check command
var preflightCmd = &cobra.Command{
	Use:   "preflight",
	Short: "Check connectivity to databases, BKRepo, and CMDB",
	Long: `Run connectivity checks against all configured external dependencies
before starting migration. This verifies:
- Source MySQL (GSEKit) database connectivity
- Target MySQL (BSCP) database connectivity
- CMDB API connectivity and authentication
- Repository (BKRepo / S3 / COS) connectivity and authentication`,
	PreRunE: requireConfig,
	Run: func(cmd *cobra.Command, args []string) {
		report := migrator.Preflight(cfg)
		migrator.PrintPreflightReport(report)
...
func init() {
	...
	migrateCmd.Flags().StringVar(&bizIDs, "biz-ids", "",
		"Comma-separated list of business IDs to migrate (required)")
	_ = migrateCmd.MarkFlagRequired("biz-ids")

	validateCmd.Flags().StringVar(&bizIDs, "biz-ids", "",
		"Comma-separated list of business IDs to validate (required)")
	_ = validateCmd.MarkFlagRequired("biz-ids")

	cleanupCmd.Flags().BoolVarP(&forceCleanup, "force", "f", false, "Skip confirmation prompt")
	cleanupCmd.Flags().StringVar(&bizIDs, "biz-ids", "",
		"Comma-separated list of business IDs to cleanup (required)")
	_ = cleanupCmd.MarkFlagRequired("biz-ids")
