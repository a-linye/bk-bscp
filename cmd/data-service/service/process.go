...
func (s *Service) OperateProcess(ctx context.Context, req *pbds.OperateProcessReq) (*pbds.OperateProcessResp, error) {
	...
	if req.OperateRanges != nil {
		environment = req.OperateRanges.GetEnvironment()
	}
...
func getProcessesAndInstances(kt *kit.Kit, dao dao.Set, req *pbds.OperateProcessReq) (
	...
	if req.OperateRanges != nil {
		return getByOperateRanges(kt, dao, req.BizId, req.OperateRanges)
	}
...
func buildOperateRange(processes []*table.Process, req *pbds.OperateProcessReq) table.OperateRange {
	...
	// 仅插件操作需要构建完整操作范围
	if req.OperateRanges != nil {
		operateRange.SetNames = []string{req.OperateRanges.GetSetName()}
		operateRange.ModuleNames = []string{req.OperateRanges.GetModuleName()}
		operateRange.ServiceNames = []string{req.OperateRanges.GetServiceName()}
		operateRange.ProcessAlias = []string{req.OperateRanges.GetProcessAlias()}
		operateRange.CCProcessID = []uint32{req.OperateRanges.GetCcProcessId()}
	} else {
		for _, process := range processes {
