...
func mig20250923114027Up(tx *gorm.DB) error {
	...
	type ProcessInstances struct {
		...
		TenantID        string     `gorm:"column:tenant_id;type:varchar(255);not null;index:idx_tenantID_bizID_processID,priority:1;default:default" json:"tenant_id"`
		BizID           uint       `gorm:"column:biz_id;type:bigint unsigned;not null;index:idx_tenantID_bizID_processID,priority:2;comment:业务ID" json:"biz_id"`         // 业务ID
		ProcessID       uint       `gorm:"column:process_id;type:bigint;not null;index:idx_tenantID_bizID_processID,priority:3;comment:关联的process表ID" json:"process_id"` // 关联的process表ID
