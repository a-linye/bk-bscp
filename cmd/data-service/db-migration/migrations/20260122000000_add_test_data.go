...
func mig20260122000000Up(tx *gorm.DB) error {
	...
	if err := tx.Exec(`INSERT INTO strategies (id, name, release_id, as_default, scope, namespace, memo, pub_state, biz_id, app_id, strategy_set_id, creator, reviser, created_at, updated_at) 
		VALUES (?, 'default-strategy', ?, true, null, '', 'default publish strategy', 'published', ?, ?, ?, ?, ?, ?, ?)`,
		testStrategyID, testReleaseID, testBizID, testAppID, testStrategySetID, creator, creator, now, now).Error; err != nil {
		return err
	}
