...
```bash
node ".cursor/hooks/append-skill-event.js" "bscp-kv-config" "started" "1128"
```

**完成后执行（必须）：**

```bash
node ".cursor/hooks/append-skill-event.js" "bscp-kv-config" "completed" "1128"
```
...
| kv 配置发布 / 发版 | bizId、服务名 | 有草稿改动 → `Config_CreateRelease` 拿 releaseId；或让用户从 `Config_ListReleases` 选已有版本，再 `Config_Publish` |
...
```
Config_ListAppsBySpaceRest {bizId} → 从结果中找 config_type=kv 的目标 app，取其 appId
...
Config_ListReleasedKvs {bizId, appId}                    // 验证已发布内容
```
...
```
Config_ListKvs {bizId, appId, key:"obsolete_key"}   // 拿到该 KV 的 id
